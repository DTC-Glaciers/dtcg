...
class DatacubeWGMS:
    ...
    def __init__(self):
        ...
        fp_glacier_ids = utils.file_downloader(base_url + "glacier_id_lut.csv")
        fp_mb_dtcg = utils.file_downloader(base_url + "WGMS_MB-DTC-Glaciers.csv")

        self.df_glacier_ids = pd.read_csv(fp_glacier_ids)[
            ["NAME", "WGMS_ID", "RGI60_ID"]
        ]
        self.df_wgms_mbs = pd.read_csv(fp_mb_dtcg)[
            [
                "glacier_id",
                "glacier_id.short_name",
                "year",
                "annual_balance",
                "annual_balance_unc",
            ]
    ...
    def get_wgms_mb(self, rgi_id, default_uncertainty):
        ...
        df_mbs = df_mbs.rename(
            columns={"annual_balance": "wgms_mb", "annual_balance_unc": "wgms_mb_unc"}
        ).set_index("year")
        ...
        df_mbs["wgms_mb_unc"] = np.where(
            np.isnan(df_mbs["wgms_mb_unc"]), default_uncertainty, df_mbs["wgms_mb_unc"]
        )

        return df_mbs

    def add_data(self, datacube, gdir):
        ...
        try:
            ...
            datacube["t_wgms"].attrs = {"long_name": "Year of WGMS observations"}
