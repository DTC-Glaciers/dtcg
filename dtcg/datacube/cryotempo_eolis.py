...
def convert_gridded_dataframe_to_array(
    ...
    x and y extent in the gridded dataframe and the provided resolution, and is
    populated with the sparse gridded data
    from the dataframe.

    Parameters
    ----------
    gridded_df : pd.DataFrame
        DataFrame containing sparse gridded data, including coordinate columns
        and value columns.
    value_column_names : list[str]
...
def prepare_eolis_metadata(
    specklia_source_data: list[dict], preliminary_dataset: bool = True
) -> dict:
    ...
    preliminary_dataset : bool, optional
        Whether to return all metadata from the first product without parsing,
        by default True.

    Returns
    -------
    dict
        Dictionary of static metadata and product-specific attributes, grouped
        by filename.
    """
    ...
    metadata = {
        k: first_source.get(k) for k in EOLIS_STATIC_KEYS if k in first_source
    }
    product_attributes = {}
...
def create_query_polygon(oggm_ds: xr.Dataset) -> shapely.Polygon:
    """Create a WGS84-aligned polygon bounding box from the spatial extent of an
    OGGM dataset.
    ...
    -------
    shapely.Polygon
        Polygon in EPSG:4326 (lat/lon) describing the outer bounds of the OGGM
        dataset.
    """
    # Create query polygon for Specklia, matching the extent of the oggm data
    # cube and then reprojected to WGS84
...
def retrieve_data_from_specklia(
    ...
    min_timestamp : int, optional
        Optional minimum timestamp to filter results, by default None. If not
        set then the minimum timestamp of the dataset in specklia will be used.
    max_timestamp : int, optional
        Optional maximum timestamp to filter results, by default None. If not
        set then the maximum timestamp of the dataset in specklia will be used.
...
def retrieve_prepare_eolis_gridded_data(
    oggm_ds: xr.Dataset, grid: Grid
) -> xr.Dataset:
    """Retrieve EOLIS gridded elevation change data and resample it to the given
    OGGM grid.
