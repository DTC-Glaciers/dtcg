...
def validate_with_sentinel(l1_datacube, l2_datacube, validation_period=None,
    ...
    if 'sfc_type_snowline' in l1_datacube:
        ...
        yrs, mnths, dys = utils.floatyear_to_date(model_floatyears_all,
                                                  return_day=True)
        ...
        common_dates = common_dates[~np.isnan(
            model_snowline.sel(member='0.5').values)]
        # exclude nan values of obseravation
        sentinel_snowline = sentinel_snowline.sel(t_sfc_type=common_dates)
        common_dates = common_dates[~np.isnan(
            sentinel_snowline.sel(snowcover_frac=0.5).values)]
        # finally select data
        # obs
        sentinel_snowline = sentinel_snowline.sel(t_sfc_type=common_dates)
        # model
        model_snowline_q_levels = [float(q) for q in model_snowline.member
                                   if q != 'Control']
        model_snowline_q_levels_sorted = sorted(model_snowline_q_levels)
        model_snowline_q_levels_sorted_str = [str(q) for q in model_snowline_q_levels_sorted]
        model_snowline = model_snowline.sel(
            time=get_model_floatyears(common_dates),
...
def plot_sentinel(l1_datacube, datatree, l2_name_list, x_lim=None,
                  dh_special=20):
    if 'sfc_type_snowline' not in l1_datacube:
        raise ValueError("For Sentinel2 plot we need the 'sfc_type_snowline' "
    ...
    inf_values = eval(sentinel_snowline.attrs['inf_values'],
                      {"np": np, "inf": np.inf})
    for l2_name in model_snowlines:
        model_inf_values = eval(model_snowlines[l2_name].attrs['inf_values'],
                                {"np": np, "inf": np.inf})
    ...
    legend_labels.append(
        'Sentinel2, snowcover area fraction of 50% (dot) and 25% and 75% '
        '(errorbar)')
    ...
    y_lims = ax.get_ylim()
    line = ax.axhspan(y_lims[0], inf_values[-np.inf] + dh_special,
                      color='lightblue', alpha=0.7, zorder=0)
    legend_handles.insert(0, line)
    legend_labels.insert(0, 'Completely snow covered')
    line = ax.axhspan(inf_values[np.inf] - dh_special, y_lims[1],
                      color='red', alpha=0.4, zorder=0)
    legend_handles.insert(0, line)
