...
def get_cryosat_data(l1_datacube=None, l2_datacube=None, baseline_date="2011-01-01"):
    """
    ...
    if l1_datacube is not None:
        ...
        baseline_elev = cryosat_elev.sel(
            t=np.array(baseline_date, dtype="datetime64[D]"), method="nearest"
        ).values
        ...
        if isinstance(cryosat_elev_unc.t.values[0], np.datetime64):
            ...
            dates = np.array(
                [datetime.fromtimestamp(t, tz=UTC) for t in cryosat_elev_unc.t.values]
            )
        ...
        relative_dt = [relativedelta(data_point, data_baseline) for data_point in dates]
        k = np.array([abs(rel_dt.years * 12 + rel_dt.months) for rel_dt in relative_dt])
        ...
        cryosat_elev_unc = np.sqrt(
            ...
            - 2
            * correlation_coeff
            * cryosat_elev_unc_baseline.values
            * cryosat_elev_unc
        )
    ...
    if l2_datacube is not None:
        ...
        model_elev = (
            model_volume - model_volume.sel(time=baseline_float, method="nearest")
        ) / l2_datacube["monthly"].area
        model_elev = sort_quantiles_keep_control_last(model_elev.isel(rgi_id=0).load())
        returns.append(model_elev)

    return returns


def sort_quantiles_keep_control_last(
    da: xr.DataArray, control_label="Control"
) -> xr.DataArray:
    ...
    if da_control is not None:
        da_out = xr.concat(
            [da_q_sorted_values, da_control.expand_dims(member=[control_label])],
            dim="member",
        )
    else:
...
def validate_with_cryosat(
    ...
    **kwargs,
    ...
    if "eolis_elevation_change_timeseries" in l1_datacube:
        ...
        # model
        model_elev_q_levels = [float(q) for q in model_elev.member if q != "Control"]
        ...
        model_elev = model_elev.sel(time=years, member=model_elev_q_levels_sorted_str)
...
def plot_cryosat(l1_datacube, datatree, l2_name_list, baseline_date="2011-01-01"):
    if "eolis_elevation_change_timeseries" not in l1_datacube:
        raise ValueError(
            "For CryoSat2 plot we need the "
    ...
    cryosat_floatyrs = utils.date_to_floatyear(
        ...
        m=(cryosat_dates.astype("datetime64[M]").astype(int) % 12) + 1,
        d=(
            cryosat_dates.astype("datetime64[D]")
            - cryosat_dates.astype("datetime64[M]")
        ).astype(int)
        + 1,
    )
    add_line_with_unc(
        ax=ax,
        x=cryosat_floatyrs,
        y=cryosat_elev.values,
        y_unc=cryosat_elev_unc.values,
        c=c_cryosat,
        label="CryoSat2",
    ...
    for l2_name, c in zip(l2_name_list, color_palette[1:]):
        ...
        model_elev = get_cryosat_data(
            l2_datacube=l2_datacube, baseline_date=baseline_date
        )[0]
